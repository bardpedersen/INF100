%Christine Sande Isaksen & Bård Tollef Pedersen
...
xlabel('x'), ylabel('y(x)')
g=inline('-0.5*exp(x/2).*sin(5*x) + 5*exp(x/2).*cos(5*x) + y','x','y');
dirfield(g,0:0.1:8,-40:1:80)
...
%% Oppgave 3
% For å konkludere kan en se at rungekutta4 gir mest nøyaktig svar da denne funksjonen
% har flere steg enn Eulers metode. Grafen vil deretter visualisere nøyaktigheten av
% disse to funksjonene. Derfor er Runge-Kutta metodene det beste alternativet for å 
% løse initialverdiproblemet. 